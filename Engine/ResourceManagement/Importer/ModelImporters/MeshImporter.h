...
private:
	FileData CreateBinary(std::vector<Mesh::Vertex> && vertices, std::vector<uint32_t> && indices, std::vector<Mesh::MorphTarget> && morph_target) const;
	...
	std::vector<Mesh::MorphTarget> GetMorphTargets(const aiMesh* assimp_mesh, const aiMatrix4x4& node_transformation) const;
};
